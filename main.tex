...
%
Single nucleotide polymorphism (SNPs) were identified after read alignment to the AgamP3 reference genome \cite{Holt2002} (Methods: SNP discovery, filtering and phasing).
%
After rigorous data quality control, utilising whole genome data from parents and siblings of lab-colony experimental crosses to quantify error rates, we identified a total of 139.7Mbp 
(60.6\% of the reference genome) that were accessible and from which SNP data could be taken for analysis of population variation (Methods: Genome accessibility).
%
Analysis of the AG1000G Phase 1 sequencing data found a slightly higher proportion of the genome to be accessible, 141.5Mbp (61.4\% of the reference genome \cite{Ag1000gConsortium2017}), but this was expected as Mendelian errors in the offspring of lab crosses were used to evaluate accessibility, and more crosses contributed to this evaluation in Phase 2 (11 crosses were used here compared to just four in Phase 1).
%
From these accessible regions of genome we identified 57,837,885 high-quality SNPs, of which 24\% were classed as multi-allelic (three or more alleles), generating an average of one variant allele every 1.9 bases of accessible genome 
(decreasing from one in every 2.2 bases in Phase 1 \cite{Ag1000gConsortium2017}). 
%
...
%
Briefly, for phase 2 we repeated the phase 1 acccessibility analyses \cite{Ag1000gConsortium2017} with 1142 samples and the additional Mendelian error information provided by the 11 crosses (in phase 1 there were four crosses).
%
...
%
We also included publicly available repeat annotations for the AgamP3 genome in the analysis of genome accessibility. 
%
These comprise annotations from three algorithms: RepeatMasker, Tandem Repeat Finder (TRF) and DUST. 
%
The DUST algorithm was originally a module within BLAST \cite{altschul1990} and is designed to mask low complexity sequences only. 
%
The RepeatMasker software \cite{smit2015} is primarily designed for masking interspersed repeats matching a library of known repeat sequences (transposable elements \textit{etc}.). 
%
TRF \cite{benson1999} specializes in locating tandem repeats.
%%
%
...
%
The first property we analysed was Mendelian error, \textit{i.e.}, the fraction of putative SNPs called with any non-Mendelian genotypes in one or more of the 11 crosses. 
%
...
and hence Mendelian errors are a good indicator of poor quality variant calls \cite{laurie2010}. 
%
The second property was the transition/transversion ratio of putative SNPs discovered in the 1142 unrelated individuals from natural populations. 
%
...
of poor variant quality \cite{guo2014}. 
%
For the numerical factors (No Coverage, Low Coverage, etc.) we looked at which parts of the numerical range were associated with high Mendelian error rates and/or low Ti/Tv. 
%
...
This classified 61\% (140Mbp) of the genome as accessible, including 91\% (18Mbp) of the exome and 58\% (121Mbp) of non-coding positions.
...
%
Briefly, sequence reads were aligned to the AgamP3 reference genome using \texttt{bwa v0.6.2} and SNPs were discovered using \texttt{GATK Unified Genotyper 2.7.4} following best practice recommendations.
%
Discovered variants were subject to quality control filters as described below.
%%
...
%
The 377 additional individuals in phase 2 resulted in >5 million more filter-passing SNPS than in phase 1 \cite{Ag1000gConsortium2017}.
%
