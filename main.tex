...
%
An important question currently facing malaria vector control programmes concerns the procurement and deployment of "next-generation" LLINs which combine pyrethroid with a piperonyl butoxide (PBO) synergist which counteracts metabolic resistance caused by cytochrome P450 genes.
%
There are no published markers for P450-based pyrethroid resistance, however it is known that P450 resistance can be achieved via over-expression of P450 genes, which in turn can be achieved by copy number amplification of the relevant gene within the genome \cite{riveron2014, muller2008}.
%
...
%
These loci have also been found under recent positive selection in multiple populations \cite{Ag1000gConsortium2017}, and have been associated with pyrethroid resistance in other studies (\textit{Cyp6} \cite{nikou2003, edi2014, faucon2015}, \textit{Cyp9} \cite{main2018, tchigossou2018, vontas2018}).
%
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Pyrethroid resistance genotypes. The geographical distribution of four pyrethroid insecticide resistance genotype states for all 1142 Phase 2 wild caught mosquitoes are shown by country across Sub-Saharan Africa. The four pie chart colours represent geneotype state: purple - these individuals we either homozygous or heterozygous for at least one of the two widespread \textit{kdr} pyrethroid target site mutations \textit{Vgsc-995F/S}; yellow - these individuals carried any duplication within genes contained in either the \textit{Cyp6} or \textit{Cyp9} gene clusters; orange - these individuals carried at least one \textit{kdr} mutation and one \textit{Cyp} cluster duplication; grey - these individuals carried no known pyrethroid resistance genotypes (no \textit{kdr} or \textit{Cyp} duplications).}
	\label{ir}
\end{figure}
%%
...
%%
Due to the skill, time and equipment costs of insecticide resistance bio-assays, molecular assays of target-site mutations are often used as a proxy for resistance to pyrethroids in natural populations \textit{e.g.} \cite{pinto2006, wilson2018} but see \cite{donnelly2009}, either due to the assumption that this mode of resistance is dominant or possibly due a historical lack of amplification markers for high-throughput study (remedied by @@Lucas et al??).
%
However, combining both target-site and amplified P450 loci in one pan-African pyrethroid resistance analysis revealed several novel findings which may call into doubt the efficacy of the \textit{kdr} proxy.
%
Target-site mutations may be good resistance indicators in certain parts of Africa, \textit{e.g.} in Gabon and Cameroon.
%
Unfortunately, it is clear from Figure \ref{ir} that genomic amplifications at loci associated with pyrethroid resistance are widespread across much of Sub Saharan Africa, often in combination with \textit{kdr} \textit{e.g.} Guinea, Burkina Faso, C\^{o}te d'Ivoire and Uganda.
%
More concerning, regarding reliance upon target-site proxies of resistance, are samples collected in the Far West of Africa: Guinea Bissau and The Gambia.
%
These may be considered pyrethroid susceptible as no \textit{kdr} mutations are found, yet carry high frequencies of metabolic resistance markers (Figure \ref{ir}); a finding which highlights the importance of locating, understanding and developing high throughput assays for amplifications as part of malaria vector control efforts.
%%

%%
Our pyrethroid resistance genotype analysis also highlights striking features of population structure (or lack thereof).
%
...
%
The mainland central/western populations appear to fall into three groupings of pyrethroid resistance loci, the Far-West where only P450 amplifications were found, western populations (Burkina Faso, Guinea, C\^{o}te d'Ivoire) where most individuals carry both \textit{kdr} and P450 amplification, and the more central population of Ghana, Gabon and Angola when target-site mutations are dominant.
%
Interestingly, this pattern of pyrethroid resistance associated genotype sharing is not found in mainland East African populations.
%
The majority of Ugandan mosquitoes sampled carried both \textit{kdr} and a P450 amplification whereas in Kenya \textit{kdr} was dominant and there was only low frequencies of P450 amplifications found.
%
Off-shore, the analysis reveals that the two (new to Phase 2) island populations, Bioko and Mayotte do not carry either of the main \textit{kdr} mutations or appear to have amplifications in the P450 genes investigated here.
%%
...
2.assay discussion 
Due to the skill and equipment costs of insecticide resistance bio-assays, high throughput molecular assays of target-site mutations are often used as a proxy for resistance to pyrethroids in natural populations \textit{e.g.} \cite{pinto2006, wilson2018} but see \cite{donnelly2009}.
%
Using these markers as a proxy may be due to the assumption that the target-site mode of resistance is dominant or possibly due to an historical lack of amplification markers for high-throughput metabolic resistance study (remedied by @@Lucas et al??). 
%
Our findings here reveal how risky this proxy may be, particularly if the natural populations have not recently had any bio-assays conducted.
%
Two of the countries we sampled from show high frequency resistance associated P450 amplifications in the absence of \texit{kdr} target-site mutations.
%
The risk of national malaria control programmes incorrectly advocating the use of pyrethroids based only on molecular target-site markers may be high, howevever, here we demonstrate that WGS data can be utilised to investigate metabolic alongside target-site pyrethroid resistance (see Lucas et al for in depth analysis).
%%
