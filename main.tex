...
%
The widespread use of chloroquine and DDT in the 1950’s led to high levels of drug and insecticide resistance, and the same pattern has been repeated for other first-line antimalarial drugs and insecticides.
%
Over the past 15 years, mass distribution of pyrethroid-treated bednets in Africa and worldwide use of artemisinin combination therapy (ACT) initially led to substantial reductions in malaria prevalence and mortality, but the rate of progress has now tailed off, with rising levels of ACT resistance in Southeast Asian parasites and of pyrethroid resistance in African mosquitoes.
%
A deep understanding of local patterns of resistance and the continually changing nature of the local parasite and vector populations is necessary to
manage the use of drugs and insecticides and to deploy public health resources for maximum sustainability and impact.
%%
...
Kai intro version.
%%
During the 1950s, the widespread use the insecticide DDT and the antimalarial drug chloroquine in the fight against malaria demonstrated both the strength that anthropogenic pressure can exert and the speed at which selection can generate phenotypes to counteract it.
%
As new first-line insecticides and antimalarials have been developed and deployed, this pattern has been repeated for over half a century. 
%
However, the last 15 years witnessed a change in fortunes, mass bed-net distributions and artemisin combination therapy (ACT) saw substantial gains with large reductions in malaria prevalence \cite{Bhatt2015} (@@get a citation for parasite).
%
2018 alone saw 172 million long-lasting insecticide treated nets (LLINs) distributed for free in sub-Saharan Africa.
%
Unfortunately the levels of pyrethroid IR have become so widespread that there are fears of vector control failure and ACT resistance is now prevalent in South East Asia and spreading fast.
%
With pyrethroids still currently used in all LLINs and no new anti-malarials, it is clear esistance surveillance is therefore essential to maximise the impact and sustainability of national malaria control programs (NMCPs). 
%%
...
%
resistance susceptibility testing with insecticide coated paper (\textit{e.g.} WHO tube assay \cite{world2016test}) and/or the detection of genetic loci associated with resistance via molecular assays (\textit{e.g.} PCR \cite{Ranson2000} and TaqMan \cite{bass2007}).
%
Measuring the insecticide resistance phenotypes of live mosquitoes samples, provides an almost real-time picture (perhaps just one generation removed) of the strength of resistance.
%
All soluble insecticides can be tested with no prior knowledge about potential modes of resistance or of causative resistance loci.
%
...
%
Detecting a resistance phenotype within a mosquito population in this way may also be difficult unless resistance is widespread (Lucas 2019 - Bioxiv).
%%

%%
Molecular assays can, conversely, be quickly run on dead samples collected directly from natural populations without requirement for breeding the high numbers of mosquitoes that are necessary for resistance assays plus reagents and equipment are relatively inexpensive. 
%
The specificity of molecular assays, which can indicate the genetic loci SNP contributing to resistance, is also restrictive, as only known resistance markers can be tested.
%
Historically the development and validation of new molecular markers is slow, although several have been identified for \textit{Anopheles gambiae/coluzzii} in the last 20 years \cite{du2005, jones2012, martinez1998, mitchell2014, ranson2000, riveron2014, weetman2018, weill2003}.
%
However, these markers explain just a fraction of the variance in resistance phenotype \cite{donnelly2009, donnelly2016}, for example a combination of three molecular markers were able to explain ~50\% of the variation of resistance to the insecticide DDT \cite{mitchell2014}.
%%


%%
The \textit{Anopheles gambiae} 1000 Genomes Project (Ag1000G), an international collaboration, was established with the aim of using a catalogue of deep-sequencing-generated genomic variation from natural mosquito populations to better understand and thus more effectively combat the principal African malaria vectors.
%
These data allow the mosquito genome to be scanned for genetic features that may be associated with insecticide resistance.
%
Selection scans can reveal signatures in the genome that may harbour...
%
Of the nn variants for which exists molecular assays (see above), only nn are associated 

%
...but because both SNP and CNV information can be extracted from deep sequencing, these data allow investigation into these different modes of resistance.
%
...
%
PBO enhances the potency of certain insecticides (including pyrethroids), where resistance to these chemicals is cytochrome P450 mediated (metabolic resistance), though where resistance is purely target-site mediated .
%
...
%%
Here, we describe the population diversity, structure and insecticide resistance marker status of the vector populations publicly available as the Ag1000G Project Phase 2 data release.
%
Analyses reveal the breadth of demography and evolutionary histories now represented in these data.
%
Investigations into both tar and illustrate the power of this genomic resource to inform malaria eradication efforts.
%%
...
%%
%%
GENOMICS AND THE AG1000G Project - see pf6
%
genomics - scan whole genomes for signals of potential insecticide resistance >> suggestive of both insecticides involved and the mode of resistance (NEED TO GET <MODE> IN EARLIER - PBO NETS!!). These can, overtime, be moclecularly validated and ground truthed using traditional methods, but genomics allows these approximations/estimations of resistance to keep pace with the fast changing evolutionary landscape...
...
%1. what is phase 2
%Phase 2 of the Ag1000G project sees an add

%2.
%Good data - increase in coluzzi, island pop and reference genome %divergence. 

%Re-analysis of qc demonstrated robustness, the optimum cutoffs from %phase 1 remain qualitatively unchanged with addition on of new %samples and variation.



\subsection*{isolation by distance discussion}

%%
Research into the migration of mosquitoes dates back to Ronald Ross, shortly after his discovery that mosquitoes transmitted malaria over 100 years ago \cite{ross1905, ross1911}, and interest in the topic has not waned since then \textit{e.g.} \cite{service1997,dao2014}.
%
Understanding this aspect of basic biology is important for optimising many aspects vector control, one currently pressing concern is the application of gene-drive to vector control.
%
Here, mosquito migration affects spread of the driven gene through populations \cite{eckhoff2017} and potentially the spread of drive-resistant alleles into susceptible populations (\cite{Ag1000gConsortium2017}. 
%
The expanded geographical sampling in Phase 2 allowed fine-scale analysis of how geographic distance between populations is related to genetic distance (isolation by distance), enabling inference of mosquito migration.
%
We found much lower isolation by distance in \textit{An. gambiae} than \textit{An. coluzzii}, suggesting long distance gene flow, and therefore long distance migration in the former but not the latter species.
%
West african gambiae's connectedness makes it more susceptible to gene-drive escaping and to resistnace arriving... 
%
Interestingly these findings are in agreement with conclusions made from the results of a study using time-series analyses (collecting 40,000 mosquitoes over five years), that \textit{An. gambiae} migrates long distances to repopulate areas after dry seasons, where as \textit{An. coluzzii} aestivates \textit{in situ} \cite{dao2014}.
%

\subsection*{cas9}
%
Continuing with the theme of gene-drive, whole genome sequencing allowed us to investigate genetic variation in and around the dsx gene, a target for gene-drive modification.
%
Our findings of ...
RNA binding constraint on non-coding positions may be generating these unusual patterns of diversity. Other regions of the genome displaying the 
%%

\subsection*{ir}
%%
With the reliance on pyrethroid insecticides for ITNs and the ubiquity of pyrethroid resistance \cite{Hemingway2016}, it is essential for NMCPs to be able to access information about which regions might benefit from new (and more expensive \cite{churcher2016}) PBO bed-net roll-out.
%
...
%
It is likely that the four loci we analysed do not explain all phenotypic variation in all locations \cite{donnelly2016, mitchell2014}, and although all loci involve resistance associated genomic regions, the specific P450 CNVs used here have yet to undergo molecular validation \cite{lucas2019}.
%
...
%%
Here, a brief overview of this rich data resource, demonstrates how the Ag1000G project data can quickly, easily and relatively cheaply inform gene-drive vector control development, PBO net-rollout and insecticide choice.
%
Our findings reiterate the value of large publicly available genomic data, that can inform many aspects of malaria control while filling gaps in our understanding mosquito evolution and  elucidating basic biological/ecological processes.
%%
