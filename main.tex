...
%
Both counts and fraction of the genome carrying contiguous runs of homozygosity (ROH) can be indicative of inbreeding.
%
A previous analysis of Phase 1 populations \cite{Ag1000gConsortium2017}, saw Kenya stand out with ROH tracts taking up to ~60\% of the genome, a much higher percentage than all other natural populations and similar to the Mali lab colony samples (these were parents of colony cross offspring) (Figure \ref{div}c). 
%
The additional colonies, and therefore colony cross parents, available for analysis in Phase 2 reveal a much broader spread in both count and fraction ROH between colonies, suggestive of different histories.
%
Ghana colony samples have some of lowest ROH counts and genome fraction found, even compared to wild caught samples, whereas some Kisumu colony samples have the highest counts and genomic fraction of ROH.     
%
ROH values from the Phase 2 novel island population of Mayotte were also striking, slightly elevated ROH fractions alongside the highest ROH counts found in natural populations.
%%

%%
Identity by descent (IBD) metrics... (Figure \ref{div}d)@@Nick?
%%
...
%
To assess how geographic distance relates to genetic differentiation we calculated Wright's "Neighborhood Size" \cite{wright1946isolation} for \textit{A. gambiae} and \textit{A. coluzzii} populations in West Africa (Figure \ref{fig:ibd_fig}a).
%
...
%
We used Rousset's \cite{rousset1997genetic} method for estimating neighborhood size based on a regression of normalized $F_{st}$ against the logarithm of geographic distance, and calculated this statistic in 200,000 base pair windows across the genome for all pairs of localities in western Africa, excluding Angola. 
%
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Isolation by distance in west African \textit{Anopheles}. \textbf{(a)} Study region and pairwise $F_{st}$. \textbf{(b)} Regressions of average genome-wide $F_{st}$ against geographic distance, following Rousset \cite{rousset1997genetic}. Neighborhood size is estimated as the inverse slope of the regression line. \textbf{(c)} Difference in neighborhood size estimates by species, with each point represenging a 200kb region of the genome. \textbf{(d)} Neighborhood size estimates across the genome, demonstrating heterogeneity relating to inversions (grey bars) and sites implicated in insecticide resistance (black triangles).}
	\label{fig:ibd_fig}
\end{figure}


%%
We found that average neighborhood sizes are lower in \textit{A. coluzzii} (median=84.7) than in \textit{A. gambiae} (median=260.5; Figure \ref{fig:ibd_fig}b, c), reflecting stronger population structure and potentially lower dispersal in \textit{A. coluzzii} populations.
%
Though both figures are strikingly low relative to census population sizes, our estimate for \textit{A. gambiae} is quite similar to a recent study in a Malaysian population of \textit{A. aegypti} in which neighborhood size was estimated at 268 \cite{jasper2019genomic}.
%%
...
%
For example, the large 2La inversion on chromosome 2L coincides with a roughly 10-fold drop in neighbourhood size relative to surrounding regions of the genome in both \textit{A. coluzzii} and \textit{A. gambiae}.
%
...
%%
Loci involved in insecticide resistance also reflected departures from average levels of isolation by distance.
%
In \textit{A. gambiae} the lowest neighborhood sizes across the genome occur in the region surrounding the \textit{VGSC} locus on chromosome 2L.
%
This region has experienced strong recent selection as a result of insecticide use, with haploptype structure reflecting a series of recent soft sweeps in populations across Africa \cite{anopheles2017genetic}.
%
...
%
However, we observed the opposite pattern in \textit{A. coluzzii}, with neighborhood size estimates near \textit{VGSC} instead strongly \emph{inflated} relative to surrounding regions.
%
This reflects high levels of genetic differentiation among some nearby populations of \textit{A. coluzzii}, which results in a flat or even negative slope of $F_{st}$ as a function of distance (Supplementary Figure \ref{fig:vgsc_ibd}).
%
Dips in estimated neighbourhood sizes were also observed near several other genes implicated in insecticide resistance (Figure \ref{fig:ibd_fig}d). 
%%
...
%TODO
%Find out what the sitch is in Drosophila: do we know how large the region is that is critical to correct splicing?
