...
\begin{figure}[H]
	...
	\end{center}
	\caption{Ag1000G Phase 2 sampling locations. Colour of circle denotes species collected at location and area represents sample size. Map colours represent ecosystem classes; dark green designates forest ecosystems; see figure 9 in \cite{sayre2013} for a compete colour legend.}
	\label{sample_map}
\end{figure}
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Principal component analysis (components 1-8) of the 1142 wild-caught mosquitoes. Bar-chart shows the percentage of variance explained by each component.}
	\label{pca}
\end{figure}
...
%%
For comparison, we performed another PCA, this time analysing CNV variation (Figure S2).
%
Initial groupings (PC1 \textit{vs.} PC2) were quite different when using CNVs, with Angola clustering with the other \textit{An. coluzzii} populations and Kenya clustering with the other two populations with uncertain species assignment (Guinea Bissau and The Gambia). 
%
These differences are, however, likely driven by variant choice; nucleotide variants are taken from regions of the genome which best represent mosquito geography \cite{Ag1000gConsortium2017}, whereas CNV variants are taken from the whole genome due to there being far fewer variants in this class.
%
Later principal components pull out Angola, Kenya, Gabon and Mayotte, which are also found when analysing nucleotide diversity, though clustering appears far less distinct with CNV data.
%%
...
%
These statistics included nucleotide diversity ($\theta_{\pi}$; Figure 5a), Watterson's theta ($\theta_{W}$; Figure S3), Tajima's $D$ (Figure 5b) and site frequency spectra (SFS; Figure S4). 
%
...
\begin{figure}[H]
	...
	\end{center}
	\caption{Genetic diversity. \textbf{(a)} Population nucleotide diversity ($\theta_{\pi}$) calculated in non-overlapping 20-kb genomic windows. \textbf{(b)} Tajima's $D$ calculated in non-overlapping 20-kb genomic windows. \textbf{(c)} Runs of homozygosity
...
%
The nucleotide variation data in this resource include 67 non-synonymous SNPs (passing quality control filters) within the \textit{Vgsc} gene that encodes the binding target for pyrethroid insecticides, of which two SNPs (\texttt{L995F}, \texttt{L995S}) are known to confer a pyrethroid resistance phenotype, and one SNP (\texttt{N1570Y}) has been shown to substantially increase pyrethroid resistance when present in combination with \texttt{L995F} \cite{jones2012}. 
%
...
%
Metabolic resistance to pyrethroids is mediated at least in part by increased expression of mixed-function oxidase (MFO) enzymes \cite{kwiatkowska13, edi2014, ngufor15, vontas2018}, and we found large numbers of independent CNV alleles at these loci \cite{lucas2019}.
%
One MFO locus is on chromosome arm 2R and overlaps a cluster of 10 cytochrome P450 genes, including \textit{Cyp6p3} previously shown to metabolise pyrethroids \cite{muller2008}.
%
The second MFO locus occurs on the X chromosome and spans a single cytochrome P450 gene, \textit{Cyp9k1} which has also previously be shown to  metabolise pyrethroids \cite{vontas2018}
%
At these loci we found genes which overlapped with at least 15 distinct CNV alleles, several of which were present in over 50\% of individuals in some populations and were associated with signatures of positive selection \cite{lucas2019}.
%
...
%
However, 81\% of Uganda individuals also had metabolic resistance, whereas only 4\% of Kenyans (two individuals) carried metabolic resistance markers, and no CNVs were found shared between these countries \cite{lucas2019}.
%
We know from previous analyses of Ag1000G data that target-site resistance has spread between these two populations \cite{Ag1000gConsortium2017}, and so it is interesting that metabolic resistance appears not to have spread.
%
...
%
Unscaled CNV variation PCAs were built from the CNV presence/absence calls \cite{lucas2019}, using the "prcomp" function in R \cite{R}.
%
