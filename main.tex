...
%
We performed whole genome sequencing of 377 individual mosquitoes collected from the wild in 3 continental locations (The Gambiae, Cote d'Ivoire, Ghana) and two offshore islands (Bioko Island, Mayotte Island), of which @@N where female and @@M where male. 
%
...
%
We then combined these data with the sequencing data previously generated during phase 1 of the project, to create a total resource of data from 1,142 wild-caught mosquitoes from 13 countries (Figure 1; Table 1) and 234 mosquitoes from 11 lab crosses (Table S1). 
%
...
%
We did not, however, assign a species to each individual mosquito based on these assays, as we found in the previous project phase that these assays are not sufficient to fully characterise species ancestry in some populations @@REF, described further below.
%%
...
\subsection*{Nucleotide variation and haplotype phasing}


%% Paragraph on SNP discovery and genome accessibility.
%
Sequence reads from all individuals were aligned to the AgamP4 reference genome @@REF and SNPs were discovered using methods previously described @@REF.
%
In total we discovered 57,837,885 SNPs passing all variant quality filters (Methods). 
%
...
%
We also performed an analysis of genome accessibility, to identify all genomic positions where read alignments were of sufficient quality and consistency to support accurate discovery and genotyping of nucleotide variation.
%
...
%% Paragraph on haplotype phasing.
%
We used high quality biallelic SNPs to construct a haplotype reference panel including all 1,142 wild-caught individuals. 
%
To improve haplotype accuracy, we used a combination of read-backed phasing and statistical phasing, as described previously @@REF. 
%
To evaluate phasing quality on the autosomes, we first phased the cross parents together with their progeny using pedigree information, then included the cross parents in the wild population phasing, and compared the results from the two phasing methods.
%
To evaluate phasing quality on the X Chromosome, we constructed synthetic female diploid data from males, then compared the phased haplotypes from the synthetic females with the known male haplotypes. 
%
@@TODO report phasing results, and compare with previous project phase?
%%


% @@TODO move results regarding divergence from reference genome until after species assignments have been made.
